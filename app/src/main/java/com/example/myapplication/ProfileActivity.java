...
public class ProfileActivity extends BaseActivity {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (firebasePhotoUrl != null && !firebasePhotoUrl.isEmpty()) {
                    Glide.with(ProfileActivity.this)
                            ...
                            .circleCrop() // Enforces circle mask inside Glide itself
                            .into(ivProfilePhoto);
                    prefs.edit()
    ...
    private void enableEditing() {
        ...
        // Swap to the active (editable) background so fields are visually and functionally enabled
        int activeBg = R.drawable.edittext_rounded;
        etDept.setEnabled(true);
        etDept.setBackgroundResource(activeBg);
        etDept.setFocusable(true);
        etDept.setFocusableInTouchMode(true);

        etYear.setEnabled(true);
        etYear.setBackgroundResource(activeBg);
        etYear.setFocusable(true);
        etYear.setFocusableInTouchMode(true);

        etPhone.setEnabled(true);
        etPhone.setBackgroundResource(activeBg);
        ...
        // Move cursor focus to Department field
        etDept.requestFocus();
    }
    ...
    private void saveProfile() {
        ...
        userRef.updateChildren(updates)
                .addOnSuccessListener(unused -> {
                    ...
                    // Revert to disabled (read-only) style
                    int disabledBg = R.drawable.disabled_edittext_bg;
                    etDept.setEnabled(false);
                    etDept.setBackgroundResource(disabledBg);
                    etYear.setEnabled(false);
                    etYear.setBackgroundResource(disabledBg);
                    etPhone.setEnabled(false);
                    etPhone.setBackgroundResource(disabledBg);
        ...
        MediaManager.get().upload(selectedImageUri)
                .callback(new UploadCallback() {
                    ...
                    public void onSuccess(String requestId, Map resultData) {
                        ...
                        // Display immediately — use signature to bypass specific URL cache, circleCrop for bounds
                        Glide.with(ProfileActivity.this)
                                .load(imageUrl)
